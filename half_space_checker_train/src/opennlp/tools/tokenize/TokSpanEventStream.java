...
public class TokSpanEventStream extends AbstractEventStream<TokenSample> {
  ...
  //
  public List<Event> events = new ArrayList<>(50);
...
//      for (Span candToken : candTokens) {
	  for (int tokenCounter = 0; tokenCounter < candTokens.length ; tokenCounter++){
		Span candToken = candTokens[tokenCounter];
        ...
        if (tokenCounter < candTokens.length-1){
        	String ctokNext = sent.substring(candTokens[tokenCounter+1].getStart(), candTokens[tokenCounter+1].getEnd());
        	String[] context = cg.getContext(ctok + ctokNext, ctok.length());
			events.add(new Event(TokenizerME.NO_SPLIT, context));
        }
